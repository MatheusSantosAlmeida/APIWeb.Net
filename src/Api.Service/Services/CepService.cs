...
namespace Api.Service.Services
{
    public class CepService : ICepService
    {
        ...
        public async Task<CepDtoCreatResult> Post(CepDtoCreat user)
        {
            var model = _mapper.Map<CepModel>(user);
            var entity = _mapper.Map<CepEntity>(model);
            var result = await _repository.InsertAsync(entity);
            return _mapper.Map<CepDtoCreatResult>(result);
        }

        public async Task<CepDtoUpdateResult> Put(CepDtoUpdate user)
        {
            var model = _mapper.Map<CepModel>(user);
            var entity = _mapper.Map<CepEntity>(model);
