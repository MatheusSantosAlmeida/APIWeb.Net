...
namespace Api.Service.Services
{
    public class MunicipioService : IMunicipioService
    {
        ...
        public async Task<MunicipioCreatDtoResult> Post(MunicipioDtoCreat user)
        {
            var model = _mapper.Map<MunicipioModel>(user);
            var entity = _mapper.Map<MunicipioEntity>(model);
            var result = await _repository.InsertAsync(entity);
            return _mapper.Map<MunicipioCreatDtoResult>(result);
        }

        public async Task<MunicipioDtoUpdateResult> Put(MunicipioDtoUpdate user)
        {
            var model = _mapper.Map<MunicipioModel>(user);
            var entity = _mapper.Map<MunicipioEntity>(model);
